...
|---|----------|---------------|------|------|
| P1 | MuMu đã cài | `MuMuManager.exe` tồn tại (mặc định `D:\Microvirt\MuMu\MuMuManager.exe`) | File tồn tại, chạy được | Thiếu file / lỗi |
...
**Mục tiêu:** Xác nhận cửa sổ emulator render theo **HardwareProfile giả tiêm vào**, không phải mặc định của MuMu — trên phiên **đã đăng nhập**.

| Bước | Hành động | Tiêu chí PASS | Tiêu chí FAIL |
|------|-----------|---------------|----------------|
| B2.1 | Kiểm tra độ phân giải: Settings → Display trong Android, hoặc `wm size` / `wm density` qua shell nếu app phơi ra. | `wm size` khớp profile (vd `1080x1920`); `wm density` khớp (vd `320`) — **không** phải mặc định host-window của MuMu | Res/dpi là mặc định MuMu, không khớp profile |
...
| B2.3 | Kiểm tra model giả: `getprop ro.product.model` (hoặc mục device-info bất kỳ). | Trả về model giả (vd `FRD-L19`) — fingerprint app nhìn thấy được | Trả về model MuMu thật / rỗng |
...
| B3.2 | Xác nhận VM đã bị hủy. | Profile về idle trong `list_profiles`; index đó không còn trong `MuMuManager.exe info -v all` | VM vẫn còn / hủy nửa vời |
...
| B3.4 | Kích hoạt **launch/provision lại đúng tài khoản** (restore snapshot mới nhất vào VM mới) qua app. | VM mới tạo có index ∉ `before` & ≠ 0; restore chạy (verify → get → restore); boot xong. MuMu có thể tái dùng index đã xóa, nên **không** coi `index ≠ index cũ` là điều kiện bắt buộc. | Provision lỗi / restore lỗi / verify sha256 FAIL |
...
## 5. B.4 — (Tùy chọn) Chuyển giữa hai profile sống (R-12 isolation)

Chỉ chạy nếu cần kiểm đa tài khoản. IPC hiện tại không có command swap trực tiếp; bài này kiểm
isolation bằng cách dừng profile A rồi chạy profile B.
...
| B4.2 | **Chưa áp dụng cho IPC hiện tại.** App hiện không có command swap trực tiếp; kiểm tra bằng cách dừng A rồi chạy profile B có snapshot login trước. | B chạy trong VM mới, restore snapshot B không lỗi | B restore lỗi / còn dữ liệu A |
| B4.3 | Mở TikTok sau khi profile B chạy. **Không nhập credential.** | TikTok đăng nhập là **B**, **KHÔNG** phải A; không thấy dữ liệu A rò rỉ | Vẫn là A / trạng thái trống / hỏi login |
...
**B.2 — Fingerprint/runtime tells**

| Bước | Kết quả | Giá trị đo được / Ghi chú |
|------|---------|---------------------------|
| B2.1 `wm size`/`wm density` ghi nhận runtime | | size=`____` dpi=`____`; MPM đã gọi `wm size/density` sau boot + sau install/restore; nếu vẫn `900x1600` thì ghi fail mitigation |
| B2.2 UI portrait, không letterbox | | |
| B2.3 `ro.product.model` ghi nhận runtime | | model=`____`; MPM re-assert resetprop sau boot + sau install/restore |
| B2.4 `android_id` trước/sau cài TikTok | | trước app có thể khớp; MPM re-apply trước start app; sau cài/chạy TikTok vẫn có thể lệch vì SSAID/GMS |
| B2.5 Magisk/Shamiko root-hide (nếu base image có full Magisk) | | chạy A.15; DenyList có TikTok, Enforce DenyList tắt, thấy Shamiko module/status |
| B2.6 Sensor entropy baseline | | chạy A.16; có accel/gyro/magnetometer, provider không lộ `goldfish/ranchu/qemu/mock` |
| **Tổng B.2** | | |
...
|------|---------|----------------------|
| B4.1 A login đúng | | |
| B4.2 Dừng A rồi chạy profile B không lỗi | | |
...
- `MuMuManager.exe` mặc định: `D:\Microvirt\MuMu\MuMuManager.exe`
...
- Owner UID: trùng UID thư mục app (`stat -c %u <marker>` == `stat -c %u /data/data/<pkg>`).
- SELinux: nếu image bật SELinux, label nên là app data; trên MuMu test hiện tại SELinux có thể Disabled nên owner mới là bằng chứng chính.
- Sai owner/label ⇒ EACCES ⇒ app coi như mất phiên ⇒ bắt đăng nhập lại.

**Ánh xạ sang test tự động** (phần này bổ sung cho, không thay thế): B.2↔A.4 (fingerprint/runtime tells); B2.5↔A.15 (Magisk/Shamiko root-hide diagnostic); B2.6↔A.16 (sensor baseline); **B.3↔A.13** (profile run/stop + app restart + restore tự động); B.3.3↔A.10 (R-15 backup-trước-hủy); retention↔A.11; storage metric↔A.14; B.4↔A.8/R-12 isolation khi có hai profile thật.
...
1. **MuMu KHÔNG hỗ trợ adb streamed install.** `adb install` mặc định dùng streamed →
...
- **android_id bị GMS/MuMu ghi đè SAU khi cài + chạy TikTok.** android_id áp được & BỀN
  ...
  trả giá trị KHÁC giá trị áp. MPM hiện re-apply `settings put secure android_id` trước khi
  trả VM cho TikTok, nhưng chưa coi là khóa cứng giá trị TikTok nhìn thấy.
- **Resolution/model đã có mitigation runtime nhưng vẫn phải đo.** MPM gọi `wm size/density`
  sau boot và sau install/restore, đồng thời re-assert `resetprop` cho `ro.product.*`; nếu B2.1
  hoặc B2.3 vẫn lệch thì ghi fail để điều tra theo log ADB/MuMu.
...
liên tiếp trong lúc gỡ lỗi đều để danh sách MuMu về đúng chỉ còn VM gốc `0,MuMu` (0 VM mồ côi).
